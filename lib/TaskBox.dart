  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: GestureDetector(
          onTap: () async {
            await Navigator.push(context, MaterialPageRoute(builder: (context) => DetailTaskScreen(id: this.id, task: this.task)));
                ...
                onDismissed: (action){
                  switch(action)
                  {
                    case SlideActionType.primary:
                      ...
                      {
                        controller.Restore(this.id);
                        Scaffold.of(context).showSnackBar(SnackBar( duration: Duration(milliseconds: 500), content: Text("Task Restored.")));
                    ...
                    case SlideActionType.secondary:
                      if(this.task.state == States.active)
                      {
                        ...
                        Scaffold.of(context).showSnackBar(SnackBar(duration: Duration(milliseconds: 500), content: Text("Task marked as Deleted.")));
                      }
                      else
                      {
                        controller.KillTask(this.id);
                        controller.RemoveFromPersistent(this.id);
                        Scaffold.of(context).showSnackBar(SnackBar(duration: Duration(milliseconds: 500), content: Text("Task deleted permanently.")));
                        viewController.Refresh(States.deleted);
                        viewController.Refresh(States.done);
                      }
                      break;
